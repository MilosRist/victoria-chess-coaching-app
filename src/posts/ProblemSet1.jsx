...
import InteractiveChessProblem from '../components/InteractiveChessProblem';
import config from '../config';
import TacticsTrainer from '../components/Chessboard'
...
const ProblemSet1 = () => {
    ...
    useEffect(() => {
        const fetchAnswers = async () => {
            try {
                const response = await fetch(`${config.apiBaseUrl}/api/answers`);
                if (!response.ok) {
    ...
    return (
        <div>
            <Header />
            <div className='flex'>
                <div className="lg:w-3/5 block mr-auto ml-auto mt-40 text-lg w-4/5">
